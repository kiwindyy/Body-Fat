...
while (length(final_variables) > 4) {
  current_model <- lm(as.formula(paste("Pct.BF ~",
                          paste(final_variables, collapse = "+"))), data = data)
  current_aic <- AIC(current_model)
  
  # Drop one variable at a time and calculate AIC
  aic_values <- sapply(final_variables, function(var) {
    temp_model <- lm(as.formula(paste("Pct.BF ~", paste(setdiff(
      final_variables, var), collapse = "+"))), data = data)
